...
async def screenshot(session, chat_id, token, login, chat_title, index, proxy, captcha_params=None):
    url_messages_send_service = 'https://api.vk.com/method/messages.sendService'
    ...
    try:
        ...
        if captcha_params: 
            params_send_service.update(captcha_params)

        async with session.post(url_messages_send_service, params=params_send_service, proxy=proxy) as response:
            result = await response.json()
            if 'error' not in result:
                print_white(f"ЛОГИН {login} СДЕЛАЛ СКРИНШОТ ЧАТА В БЕСЕДЕ {chat_title} - {index}")
            else:
                ...
                if error_code == 14:
                    ...
                    while not solved:
                        try:
                            ...
                            solved = True
                            time.sleep(3)
                            return await screenshot(session, chat_id, token, login, chat_title, index, proxy, captcha_params)

                        except Exception as captcha_error:
                            print_white(f"ОШИБОЧКА ПРИ РЕШЕНИИ КАПЧИ: {captcha_error}")

                else:
                    logging.error(f"ОШИБКА VK API ПРИ ОТПРАВКЕ СКРИНШОТА: {error_code} - {error_msg}")
...
async def run():
    ...
    async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=False), headers=headers) as session:
        while True:
            await screenshot(session, chat_id, token, login, chat_title, index, proxy)
            index += 1
            time.sleep(0.33)
