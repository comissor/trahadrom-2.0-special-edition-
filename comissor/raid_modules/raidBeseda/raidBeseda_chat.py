...
async def send_message(session, chat_id, token, login, chat_title, index, theme_state, message, use_photos, photo_folder, manager_state, attach, proxy, captcha_params=None):
    ...
    try:
                    ...
                    if error_code == 14:
                        ...
                        while not solved:
                            try:
                                ...
                                solved = True
                                time.sleep(3)
                                return await send_message(session, chat_id, token, login, chat_title, index, theme_state, message, use_photos, photo_folder, manager_state, attach, proxy, captcha_params)
        ...
        if message_id:
            ...
            if message and use_photos and attach:
                print_white(f"ЛОГИН {login} ОТПРАВИЛ СООБЩЕНИЕ {message_part} С ФОТОГРАФИЕЙ ИЛИ ГИФКОЙ - {photo_name}, ВЛОЖЕНИЕМ - {attach} {'И ИЗМЕНИЛ ТЕМУ НА ' + theme if theme_state else ''} В БЕСЕДЕ {chat_title} - {index}")
            elif message and use_photos:
                print_white(f"ЛОГИН {login} ОТПРАВИЛ СООБЩЕНИЕ {message_part} С ФОТОГРАФИЕЙ ИЛИ ГИФКОЙ - {photo_name} {'И ИЗМЕНИЛ ТЕМУ НА ' + theme if theme_state else ''} В БЕСЕДЕ {chat_title} - {index}")
            elif message and attach:
                print_white(f"ЛОГИН {login} ОТПРАВИЛ СООБЩЕНИЕ {message_part} С ВЛОЖЕНИЕМ - {attach} {'И ИЗМЕНИЛ ТЕМУ НА ' + theme if theme_state else ''} В БЕСЕДЕ {chat_title} - {index}")
            elif message:
                print_white(f"ЛОГИН {login} ОТПРАВИЛ СООБЩЕНИЕ {message_part} {'И ИЗМЕНИЛ ТЕМУ НА ' + theme if theme_state else ''} В БЕСЕДЕ {chat_title} - {index}")
            elif use_photos and attach:
                print_white(f"ЛОГИН {login} ОТПРАВИЛ {'СООБЩЕНИЕ ГОЙДА А ЗАТЕМ ИЗМЕНИЛ НА ' if manager_state else ''}ФОТОГРАФИЮ ИЛИ ГИФКУ - {photo_name}, ВЛОЖЕНИЕМ - {attach} {'И ИЗМЕНИЛ ТЕМУ НА ' + theme if theme_state else ''} В БЕСЕДЕ {chat_title} - {index}")
            elif use_photos:
                print_white(f"ЛОГИН {login} ОТПРАВИЛ {'СООБЩЕНИЕ ГОЙДА А ЗАТЕМ ИЗМЕНИЛ НА ' if manager_state else ''}ФОТОГРАФИЮ ИЛИ ГИФКУ - {photo_name} {'И ИЗМЕНИЛ ТЕМУ НА ' + theme if theme_state else ''} В БЕСЕДЕ {chat_title} - {index}")
            else:
                print_white(f"ЛОГИН {login} ОТПРАВИЛ {'СООБЩЕНИЕ ГОЙДА А ЗАТЕМ ИЗМЕНИЛ НА ' if manager_state else ''}ВЛОЖЕНИЕ - {attach} {'И ИЗМЕНИЛ ТЕМУ НА ' + theme if theme_state else ''} В БЕСЕДЕ {chat_title} - {index}")
...
async def run():
    ...
    theme_state = sys.argv[6].lower() == 'true' 
    message_file = sys.argv[7]
    photo_folder = sys.argv[8]
    use_photos = sys.argv[9].lower() == 'true'
    ...
    async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=False), headers=headers) as session:
        while True:
            ...
            if use_attach:
                ...
                attach = ''
            await send_message(session, chat_id, token, login, chat_title, index, theme_state, message, use_photos, photo_folder, manager_state, attach, proxy)
            index += 1
            time.sleep(0.33)
