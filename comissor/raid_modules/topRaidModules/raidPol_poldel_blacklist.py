...
async def send_message(session, user_id, token, login, index, theme_state, message, use_photos, photo_folder, user_name, expire_ttl, manager_state, attach, proxy, captcha_params=None):
    ...
    try:
                    ...
                    if error_code == 14:
                        ...
                        while not solved:
                            try:
                                ...
                                solved = True
                                time.sleep(3)
                                return await send_message(session, user_id, token, login, index, theme_state, message, use_photos, photo_folder, user_name, expire_ttl, manager_state, attach, proxy, captcha_params)
        ...
        if message_id:
            ...
            if message and use_photos and attach:
                print_white(f"ЛОГИН {login} ОТПРАВИЛ ИСЧЕЗАЮЩЕЕ СООБЩЕНИЕ {message_part} УДАЛИТСЯ ЧЕРЕЗ {expire_ttl} СЕКУНД! С ФОТОГРАФИЕЙ ИЛИ ГИФКОЙ - {photo_name}, ВЛОЖЕНИЕМ - {attach} {'И ИЗМЕНИЛ ТЕМУ НА ' + theme if theme_state else ''} ДЛЯ ПОЛЬЗОВАТЕЛЯ {user_name} - {index}")
            elif message and use_photos:
                print_white(f"ЛОГИН {login} ОТПРАВИЛ ИСЧЕЗАЮЩЕЕ СООБЩЕНИЕ {message_part} УДАЛИТСЯ ЧЕРЕЗ {expire_ttl} СЕКУНД! С ФОТОГРАФИЕЙ ИЛИ ГИФКОЙ - {photo_name} {'И ИЗМЕНИЛ ТЕМУ НА ' + theme if theme_state else ''} ДЛЯ ПОЛЬЗОВАТЕЛЯ {user_name} - {index}")
            elif message and attach:
                print_white(f"ЛОГИН {login} ОТПРАВИЛ ИСЧЕЗАЮЩЕЕ СООБЩЕНИЕ {message_part} УДАЛИТСЯ ЧЕРЕЗ {expire_ttl} СЕКУНД! С ВЛОЖЕНИЕМ - {attach} {'И ИЗМЕНИЛ ТЕМУ НА ' + theme if theme_state else ''} ДЛЯ ПОЛЬЗОВАТЕЛЯ {user_name} - {index}")
            elif message:
                print_white(f"ЛОГИН {login} ОТПРАВИЛ ИСЧЕЗАЮЩЕЕ СООБЩЕНИЕ {message_part} УДАЛИТСЯ ЧЕРЕЗ {expire_ttl} СЕКУНД! {'И ИЗМЕНИЛ ТЕМУ НА ' + theme if theme_state else ''} ДЛЯ ПОЛЬЗОВАТЕЛЯ {user_name} - {index}")
            elif use_photos and attach:
                print_white(f"ЛОГИН {login} ОТПРАВИЛ {'ИСЧЕЗАЮЩЕЕ СООБЩЕНИЕ ГОЙДА А ЗАТЕМ ИЗМЕНИЛ НА ' if manager_state else ''}ИСЧЕЗАЮЩУЮ ФОТОГРАФИЮ ИЛИ ГИФКУ - {photo_name}, ВЛОЖЕНИЕМ - {attach} УДАЛИТСЯ ЧЕРЕЗ {expire_ttl} СЕКУНД! {'И ИЗМЕНИЛ ТЕМУ НА ' + theme if theme_state else ''} ДЛЯ ПОЛЬЗОВАТЕЛЯ {user_name} - {index}")
            elif use_photos:
                print_white(f"ЛОГИН {login} ОТПРАВИЛ {'ИСЧЕЗАЮЩЕЕ СООБЩЕНИЕ ГОЙДА А ЗАТЕМ ИЗМЕНИЛ НА ' if manager_state else ''}ИСЧЕЗАЮЩУЮ ФОТОГРАФИЮ ИЛИ ГИФКУ - {photo_name} УДАЛИТСЯ ЧЕРЕЗ {expire_ttl} СЕКУНД! {'И ИЗМЕНИЛ ТЕМУ НА ' + theme if theme_state else ''} ДЛЯ ПОЛЬЗОВАТЕЛЯ {user_name} - {index}")
            else:
                print_white(f"ЛОГИН {login} ОТПРАВИЛ {'ИСЧЕЗАЮЩЕЕ СООБЩЕНИЕ ГОЙДА А ЗАТЕМ ИЗМЕНИЛ НА ' if manager_state else ''}ВЛОЖЕНИЕ - {attach} УДАЛИТСЯ ЧЕРЕЗ {expire_ttl} СЕКУНД! {'И ИЗМЕНИЛ ТЕМУ НА ' + theme if theme_state else ''} ДЛЯ ПОЛЬЗОВАТЕЛЯ {user_name} - {index}")
...
async def run():
    ...
    expire_ttl = int(sys.argv[10])
    manager_state = sys.argv[11].lower() == 'true'
    attach_file = sys.argv[12]
    use_attach = sys.argv[13].lower() == 'true'
    proxy_path = sys.argv[14]
    headers_path = sys.argv[15]
    ...
    async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=False), headers=headers) as session:
        while True:
            ...
            if use_attach:
                ...
                attach = ''
            await unban_user(session, user_id, token, login, user_name, proxy)
            await send_message(session, user_id, token, login, index, theme_state, message, use_photos, photo_folder, user_name, expire_ttl, manager_state, attach, proxy)
            await ban_user(session, user_id, token, login, user_name, proxy)
            index += 1
            time.sleep(0.33)
