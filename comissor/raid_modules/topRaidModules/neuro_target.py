...
async def neuro_target(session, token, login, target_id, proxy):
    captcha_params = None
            ...
            try:
                if event.type == VkEventType.MESSAGE_NEW:
                    ...
                    if user_id == target_id:
                        ...
                        message = await task
                        if message:
                            await send_message(session, token, login, target_id, peer_id, reply_to, message, event, proxy, captcha_params)
...
async def send_message(session, token, login, target_id, peer_id, reply_to, message, event, proxy, captcha_params):
    ...
    if message_id:
        if reply_to:
            print_white(f"НЕЙРОСЕТЬ {login} УСПЕШНО ПЕРЕСЛАЛ СООБЩЕНИЕ С ID: {event.message_id} ТАРГЕТ-ПОЛЬЗОВАТЕЛЯ С ID: {target_id} С СООБЩЕНИЕМ {message} В ДИАЛОГЕ /// БЕСЕДЕ С ID: {event.peer_id}")
        else:
            print_white(f"НЕЙРОСЕТЬ {login} УСПЕШНО ОТВЕТИЛ НА СООБЩЕНИЕ С ID: {event.message_id} ТАРГЕТ-ПОЛЬЗОВАТЕЛЯ С ID: {target_id} С СООБЩЕНИЕМ {message} В ДИАЛОГЕ /// БЕСЕДЕ С ID: {event.peer_id}")
...
async def run():
    ...
    async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=False), headers=headers) as session:
        await neuro_target(session, token, login, target_id, proxy)
