...
async def send_sticker(session, token, group_id, group_name, login, index, proxy, captcha_params=None):
    ...
    try:
                ...
                if error_code == 14:
                    ...
                    while not solved:
                        try:
                            ...
                            solved = True
                            time.sleep(3)
                            return await send_sticker(session, token, group_id, group_name, login, index, proxy, captcha_params)
        ...
        if message_id:
            print_white(f"ЛОГИН {login} ОТПРАВИЛ СТИКЕР {sticker_id} В ЛС ГРУППЫ {group_name} - {index}")
            return {"success": True}
...
async def run():
    ...
    async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=False), headers=headers) as session:
        while True:
            await send_sticker(session, token, group_id, group_name, login, index, proxy)
            index += 1
            time.sleep(0.33)
