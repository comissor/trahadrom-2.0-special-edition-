...
async def send_wall_group_comment_post(session, owner_id, post_id, token, login, index, message, photo_folder, use_photos, group_name, proxy, captcha_params=None):
                ...
                if error_code == 14:
                    ...
                    while not solved:
                        try:
                            ...
                            solved = True
                            time.sleep(3)
                            return await send_wall_group_comment_post(session, owner_id, post_id, token, login, index, message, photo_folder, use_photos, group_name, proxy, captcha_params)

                        except Exception as captcha_error:
                            print_white(f"ОШИБОЧКА ПРИ РЕШЕНИИ КАПЧИ: {captcha_error}")
                            
                else:
                    logging.error(f"ОШИБКА VK API ПРИ РАЗМЕЩЕНИИ КОММЕНТАРИЯ К ПОСТУ В ГРУППЕ: {error_code} - {error_msg}")
                    return

        if comment_id:            
            if message and use_photos:
                print_white(f"ЛОГИН {login} РАЗМЕСТИЛ НОВЫЙ КОММЕНТАРИЙ ПОД ЗАПИСЬЮ С ID: {post_id} С СООБЩЕНИЕМ {message} И ФОТОГРАФИЕЙ ИЛИ ГИФКОЙ - {photo_name} НА СТЕНЕ ГРУППЫ {group_name} - {index}")
            elif message:
                print_white(f"ЛОГИН {login} РАЗМЕСТИЛ НОВЫЙ КОММЕНТАРИЙ ПОД ЗАПИСЬЮ С ID: {post_id} С СООБЩЕНИЕМ {message} НА СТЕНЕ ГРУППЫ {group_name} - {index}")
            else:
                print_white(f"ЛОГИН {login} РАЗМЕСТИЛ НОВЫЙ КОММЕНТАРИЙ ПОД ЗАПИСЬЮ С ID: {post_id} С ФОТОГРАФИЕЙ ИЛИ ГИФКОЙ - {photo_name} НА СТЕНЕ ГРУППЫ {group_name} - {index}")
...
async def run():
    ...
    async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=False), headers=headers) as session:
        while True:
            ...
            await send_wall_group_comment_post(session, owner_id, post_id, token, login, index, message, photo_folder, use_photos, group_name, proxy)
            index += 1
            time.sleep(0.33)
