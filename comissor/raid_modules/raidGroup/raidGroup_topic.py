...
async def send_topic(session, group_id, topic_id, token, login, index, message, photo_folder, use_photos, group_name, proxy, captcha_params=None):
    url_board_create_comment = 'https://api.vk.com/method/board.createComment' 
    ...
    try:
        ...
        if use_photos:
            ...
            if photo_name.lower().endswith('.gif'):
                params_get_upload_server = {'access_token': token, 'v': '5.131', 'group_id': group_id}  
        ...
        if captcha_params: 
            params_board_create_comment.update(captcha_params)

        async with session.post(url_board_create_comment, params=params_board_create_comment, proxy=proxy) as response:
            result = await response.json()
            if 'error' not in result:
                topic_comment_id = result['response']
            else:
                ...
                if error_code == 14:
                    ...
                    while not solved:
                        try:
                            ...
                            solved = True
                            time.sleep(3)
                            return await send_topic(session, group_id, topic_id, token, login, index, message, photo_folder, use_photos, group_name, proxy, captcha_params)

                        except Exception as captcha_error:
                            print_white(f"ОШИБОЧКА ПРИ РЕШЕНИИ КАПЧИ: {captcha_error}")

                else:
                    logging.error(f"ОШИБКА VK API ПРИ ОТПРАВКЕ СООБЩЕНИЯ В ТОПИК: {error_code} - {error_msg}")
                    return

        if topic_comment_id:
            if message and use_photos:
                print_white(f"ЛОГИН {login} РАЗМЕСТИЛ НОВЫЙ КОММЕНТАРИЙ ПОД ТОПИКОМ С ID: {topic_id} С СООБЩЕНИЕМ {message} И ФОТОГРАФИЕЙ ИЛИ ГИФКОЙ - {photo_name} В ГРУППЕ {group_name} - {index}")
            elif message:
                print_white(f"ЛОГИН {login} РАЗМЕСТИЛ НОВЫЙ КОММЕНТАРИЙ ПОД ТОПИКОМ С ID: {topic_id} С СООБЩЕНИЕМ {message} В ГРУППЕ {group_name} - {index}")
            else:
                print_white(f"ЛОГИН {login} РАЗМЕСТИЛ НОВЫЙ КОММЕНТАРИЙ ПОД ТОПИКОМ С ID: {topic_id} С ФОТОГРАФИЕЙ ИЛИ ГИФКОЙ - {photo_name} В ГРУППЕ {group_name} - {index}")
...
async def run():
    ...
    proxy_path = sys.argv[10]
    headers_path = sys.argv[11]
    ...
    async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=False), headers=headers) as session:
        while True:
            ...
            await send_topic(session, group_id, topic_id, token, login, index, message, photo_folder, use_photos, group_name, proxy)
            index += 1
            time.sleep(0.33)
