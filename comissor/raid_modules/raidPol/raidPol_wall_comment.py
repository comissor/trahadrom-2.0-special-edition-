...
async def send_wall_post_comment(session, owner_id, post_id, token, login, index, message, photo_folder, use_photos, user_name, proxy, captcha_params=None):
    ...
    try:
        ...
        if captcha_params: 
            params_wall_create_comment.update(captcha_params)

        async with session.post(url_wall_create_comment, params=params_wall_create_comment, proxy=proxy) as response:
                ...
                if error_code == 14:
                    ...
                    while not solved:
                        try:
                            ...
                            solved = True
                            time.sleep(3)
                            return await send_wall_post_comment(session, owner_id, post_id, token, login, index, message, photo_folder, use_photos, user_name, proxy, captcha_params)

                        except Exception as captcha_error:
                            print_white(f"ОШИБОЧКА ПРИ РЕШЕНИИ КАПЧИ: {captcha_error}")

                else:
                    logging.error(f"ОШИБКА VK API ПРИ РАЗМЕЩЕНИИ КОММЕНТАРИЯ К ПОСТУ НА СТЕНЕ ПОЛЬЗОВАТЕЛЯ: {error_code} - {error_msg}")
                    return

        if comment_id:
            if message and use_photos:
                print_white(f"ЛОГИН {login} РАЗМЕСТИЛ НОВЫЙ КОММЕНТАРИЙ ПОД ЗАПИСЬЮ С ID: {post_id} С СООБЩЕНИЕМ {message} И ФОТОГРАФИЕЙ ИЛИ ГИФКОЙ - {photo_name} НА СТЕНЕ ПОЛЬЗОВАТЕЛЯ {user_name} - {index}")
            elif message:
                print_white(f"ЛОГИН {login} РАЗМЕСТИЛ НОВЫЙ КОММЕНТАРИЙ ПОД ЗАПИСЬЮ С ID: {post_id} С СООБЩЕНИЕМ {message} НА СТЕНЕ ПОЛЬЗОВАТЕЛЯ {user_name} - {index}")
            else:
                print_white(f"ЛОГИН {login} РАЗМЕСТИЛ НОВЫЙ КОММЕНТАРИЙ ПОД ЗАПИСЬЮ С ID: {post_id} С ФОТОГРАФИЕЙ ИЛИ ГИФКОЙ - {photo_name} НА СТЕНЕ ПОЛЬЗОВАТЕЛЯ {user_name} - {index}")
...
async def run():
    ...
    async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=False), headers=headers) as session:
        while True:
            ...
            await send_wall_post_comment(session, owner_id, post_id, token, login, index, message, photo_folder, use_photos, user_name, proxy)
            index += 1
            time.sleep(0.33)
