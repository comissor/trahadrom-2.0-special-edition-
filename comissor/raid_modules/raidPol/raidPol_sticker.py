...
async def send_sticker(session, token, user_id, user_name, login, index, theme_state, proxy, captcha_params=None):
    ...
    try:
                ...
                if error_code == 14:
                    ...
                    while not solved:
                        try:
                            ...
                            solved = True
                            time.sleep(3)
                            return await send_sticker(session, token, user_id, user_name, login, index, theme_state, proxy, captcha_params)
        ...
        if message_id:
            if theme_state == "True":
                ...
                print_white(f"ЛОГИН {login} ОТПРАВИЛ СТИКЕР {sticker_id} ПОЛЬЗОВАТЕЛЮ {user_name} И ИЗМЕНИЛ ТЕМУ НА {theme} - {index}")
            else:
                print_white(f"ЛОГИН {login} ОТПРАВИЛ СТИКЕР {sticker_id} ПОЛЬЗОВАТЕЛЮ {user_name} - {index}")
            return {"success": True}
...
async def run():
    ...
    async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=False), headers=headers) as session:
        while True:
            await send_sticker(session, token, user_id, user_name, login, index, theme_state, proxy)
            index += 1
            time.sleep(0.33)
