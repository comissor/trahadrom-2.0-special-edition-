...
async def send_wall_post(session, owner_id, token, login, index, message, photo_folder, use_photos, user_name, proxy, captcha_params=None):
                ...
                if error_code == 14:
                    ...
                    while not solved:
                        try:
                            ...
                            solved = True
                            time.sleep(3)
                            return await send_wall_post(session, owner_id, token, login, index, message, photo_folder, use_photos, user_name, proxy, captcha_params)

                        except Exception as captcha_error:
                            print_white(f"ОШИБОЧКА ПРИ РЕШЕНИИ КАПЧИ: {captcha_error}")

                else:
                    logging.error(f"ОШИБКА VK API ПРИ РАЗМЕЩЕНИИ ПОСТА НА СТЕНЕ ПОЛЬЗОВАТЕЛЯ: {error_code} - {error_msg}")
                    return

        if post_id:
            if message and use_photos:
                print_white(f"ЛОГИН {login} РАЗМЕСТИЛ НОВУЮ ЗАПИСЬ C ID: {post_id} С СООБЩЕНИЕМ {message} И ФОТОГРАФИЕЙ ИЛИ ГИФКОЙ - {photo_name} НА СТЕНЕ ПОЛЬЗОВАТЕЛЯ {user_name} - {index}")
            elif message:
                print_white(f"ЛОГИН {login} РАЗМЕСТИЛ НОВУЮ ЗАПИСЬ C ID: {post_id} С СООБЩЕНИЕМ {message} НА СТЕНЕ ПОЛЬЗОВАТЕЛЯ {user_name} - {index}")
            else:
                print_white(f"ЛОГИН {login} РАЗМЕСТИЛ НОВУЮ ЗАПИСЬ С ID: {post_id} С ФОТОГРАФИЕЙ ИЛИ ГИФКОЙ - {photo_name} НА СТЕНЕ ПОЛЬЗОВАТЕЛЯ {user_name} - {index}")
...
async def run():
    ...
    async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=False), headers=headers) as session:
        while True:
            ...
            await send_wall_post(session, owner_id, token, login, index, message, photo_folder, use_photos, user_name, proxy)
            index += 1
            time.sleep(0.33)
